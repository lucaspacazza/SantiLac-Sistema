...
export function ResumoDiarioView({ snapshot }: { snapshot: DashboardResumoSnapshot }) {
  ...
  const pasteurizadorDestino = pasteurizadorHash(diario.pasteurizador)
  ...
  return (
    <div className="dashboard-daily-grid">
      ...
      <article className="panel dashboard-section daily-operation-panel">
        <SectionHeader title="Resumo diario da operacao" subtitle="o dia selecionado consolidado em uma leitura so" />

        <section className="daily-pasteurizador-card">
          <div>
            <h3>Amostra do pasteurizador</h3>
            <p>o clique abre o historico com o filtro desse dia</p>
          </div>
          <PasteurizadorResumoCard pasteurizador={diario.pasteurizador} large onClick={() => openHash(pasteurizadorDestino)} />
        ...
        <div className="operational-list">
          ...
          <OperationalRow title="Producao" text={diario.producao.total_lotes > 0 ? `${diario.producao.total_lotes} lote(s) registrado(s) no dia` : 'Sem producao nova no dia'} action="Abrir producao" onClick={() => openHash(snapshot.rotas.producao)} />
          <OperationalRow title="Estoque" text="Disponibilidade atual ainda valida para decisao" action="Abrir estoque" onClick={() => openHash(snapshot.rotas.estoque)} />
          <OperationalRow title="Alertas" text={alertas > 0 ? `${alertas} item(ns) abaixo do minimo` : 'Nenhum item critico no estoque agora'} action="Abrir estoque" onClick={() => openHash(snapshot.rotas.estoque)} />
        </div>
      </article>

      <article className="panel dashboard-section daily-reading-panel">
        <SectionHeader title="Leitura do dia" subtitle="sinais operacionais e desdobramentos rapidos" />
        <div className="status-list">
          ...
          <StatusRow tag="Pasteurizador" tone={diario.pasteurizador.total_pontos > 0 ? 'ok' : 'warn'} onClick={() => openHash(pasteurizadorDestino)}>
            {diario.pasteurizador.total_pontos > 0 ? 'rodando e gravando normal' : 'sem amostras no periodo'}
          </StatusRow>
          <StatusRow tag="Producao" tone={diario.producao.total_lotes > 0 ? 'ok' : 'warn'} onClick={() => openHash(snapshot.rotas.producao)}>
            {diario.producao.total_lotes > 0 ? `${diario.producao.total_lotes} lote(s) no dia` : 'continua usando ultima base'}
          </StatusRow>
          <StatusRow tag="Estoque" tone={estoqueOk ? 'ok' : 'danger'} onClick={() => openHash(snapshot.rotas.estoque)}>
            {estoqueOk ? 'visao atual disponivel' : `${diario.estoque.abaixo_minimo} item(ns) abaixo do minimo`}
          </StatusRow>
        </div>
      </article>

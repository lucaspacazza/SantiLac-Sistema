...
export function ListagemProducoesCreme({
  items,
  pagination,
  search,
  onSearchChange,
  ...
  onOpenItem: (item: ProducaoCreme) => void
  onCreateNew: () => void
    ...
    <div className="stack">
      ...
      <section className="panel">
        ...
        <div className="table table-compact-5">
          <div className="table-head table-head-actions"><span>Data</span><span>Lote</span><span>Creme</span><span>Quantidade</span><span>Ação</span></div>
          {items.map((item) => {
            ...
            return (
              <div className={`table-row table-row-actions row-button ${!isOpen ? 'is-locked' : ''}`} key={item.id} role="button" tabIndex={0} onClick={() => onOpenItem(item)}>
                ...
                <span>{formatNumber(item.quantidade_produzida_kg, ' kg')}</span>
                {isOpen

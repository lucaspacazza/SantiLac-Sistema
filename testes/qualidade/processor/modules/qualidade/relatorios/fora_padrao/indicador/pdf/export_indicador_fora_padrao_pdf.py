...
GENERAL_EXPORTER = CURRENT_DIR.parents[1] / "geral" / "pdf"
sys.path.insert(0, str(GENERAL_EXPORTER))

from export_fora_padrao_pdf import build_pdf  # noqa: E402


OPERATION = "qualidade.relatorios.fora_padrao.indicador.pdf"
...
def export_file(input_path: Path, output_path: Path, logo_path: Path | None) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "errors": [{"code": "EXPORT_940", "message": "Falha ao gerar PDF.", "details": {"error": str(exc)}}],
        }


def main() -> int:
    parser = argparse.ArgumentParser(description="Exporta PDF de fora do padrão por indicador.")

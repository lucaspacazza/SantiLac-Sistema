...
pct exec "$FRONTEND_CT" -- sh -lc "set -e; mkdir -p \"$FRONTEND_PATH\"; cd \"$FRONTEND_PATH\"; cp -f index.html index.html.bak-\$(date +%Y%m%d%H%M%S) 2>/dev/null || true; tar -xzf \"$REMOTE_PACKAGE\" -C \"$FRONTEND_PATH\"; rm -f fabrica-sw.js fabrica.webmanifest; rm -f \"$REMOTE_PACKAGE\""
